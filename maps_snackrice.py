...
def maps_recommendations(input):
    url = generate_url({"input":input, "strictbounds":"true", "location":str(riceu_lat)+","+str(riceu_long), "radius":str(riceu_radius)},"place/autocomplete",key)
    response = requests.request("GET", url, headers={}, data={})
    response = response.json()
    recommendations = []
    for location in response["predictions"]:
        recommendations.append({"name":location["structured_formatting"]["main_text"], "place_id":location["place_id"]})
...
def servery_distances(place_id):
    ...
    url = generate_url({"destinations":destinations, "origins":"place_id:" + place_id, "mode":"WALKING"}, "distancematrix", key)
...
def optimized_servery(servery_dists, servery_scores, walking_factor):
    ...
    for i in range(len(servery_dists)):
        servery_better_scores.append(-1 * servery_dists[i] / max_dist * walking_factor + servery_scores[i] * (1-walking_factor))
